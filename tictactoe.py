# Assigment:    W02 Prove: Developer - Solo Code Submission - Tic-Tac-Toe game
# Author:       David Flores Luna

import array

winning_match_comb = [[1,2,3],[4,5,6],[7,8,9],[1,4,7],[2,5,8],[1,5,9],[3,5,7]]
...
def check_for_winner(player1, player2):
    ...
    #check
    for win_case in winning_match_comb:
        wins = True
        for val in win_case:
            wins = wins and player1[val-1] != ""
        
        if wins == True:
            #print(f"for {win_case} wins: {wins}")
            winner = 1
            break
